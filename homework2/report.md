...
|----------|----------|-----------------|-----------|-----------|
| 測試一 | 題目一<br>（Add） | `3 3 2 2 1 -4 0`<br>`2 1 1 1 0`<br>`2` | `p + q = 3x^2 + 3x - 3` | `p + q = 3*x^2 + 3*x - 3` |
| 測試二 | 題目一<br>（Mult） | `3 3 2 2 1 -4 0`<br>`2 1 1 1 0`<br>`2` | `p * q = 3x^3 + 5x^2 - 2x - 4` | `p * q = 3*x^3 + 5*x^2 - 2*x - 4` |
| 測試三 | 題目一<br>（Eval） | `3 3 2 2 1 -4 0`<br>`2 1 1 1 0`<br>`2` | `p(2) = 12` | `p(2) = 12` |
| 測試四 | 題目二<br>（>> 之後 <<） | `3 3 2 2 1 -4 0`<br>`2 1 1 1 0`<br>`2` | `p(x) = 3x^2 + 2x - 4` | `p(x) = 3*x^2 + 2*x - 4` |
| 測試五 | 題目二<br>（零多項式 I/O） | `0`<br>`0`<br>`0` | `0` | `0` |
...
2. **開發歷程**
   - **問題追蹤**：最常見錯誤是 `attach()` 未正確合併同指數，導致 Add / Mult 結果重複；以 `assert` 保證插入前後仍保持遞減序列後迅速定位。  
...
> 綜合而言，本專案驗證「抽象資料型別 → 具體資料結構 → 安全資源管理 → 完整測試」開發流程，  

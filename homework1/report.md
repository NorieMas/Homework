...
2. 非遞迴版：
   - 準備靜態陣列 `stack[MAX]` 與整數 `top` 作為手動堆疊
   - push 第一個 **Frame** `(m,n)`
   - 依三種情況調整 **新的 `(m,n)`，並 push / pop 整個 Frame**
   - 當 `top < 0` 時結束（表示最外層已取回結果）
...
**題目一：Ackermann**
1. **時間複雜度**：Ackermann 函式的時間複雜度近指數級，隨 `m`、`n` 急遽爆炸。
2. **空間複雜度**：
   - 遞迴版：系統呼叫堆疊，空間複雜度約為 $O(d)$，其中 d = `呼叫深度`。
   - 非遞迴版：以固定陣列模擬堆疊，空間複雜度為 $O(k)$，其中 k = `MAX_STACK`。


**題目一：Power Set**
1. **時間複雜度**：列舉所有子集合必須 $O(2^n)$。
2. **空間複雜度**：遞迴同時僅維持一路徑，空間複雜度為 $O(n)$。
...
1. 兩版 **Ackermann** 在 `m, n ≤ 3` 時輸出一致；遞迴版簡潔且直觀，迭代版透過手動堆疊突破語言遞迴深度限制（調整 `MAX_STACK`）。
